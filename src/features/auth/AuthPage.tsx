...
import { requestPasswordReset, signIn, signUp, validateAuthInput } from './authService'
...
export default function AuthPage() {
  ...
  const handleSubmit = useCallback(
    async (event: React.FormEvent) => {
      ...
      const validation = validateAuthInput({ email: state.email, password: state.password })
    ...
    [configured, navigate, state.loading, state.mode, state.email, state.password]
  )
      ...
      {!configured ? (
        <div className="auth-local-note" role="status">
          ...
          <p>
            ...
            <code>VITE_SUPABASE_PUBLISHABLE_KEY</code> 后即可使用邮箱登录和云端同步。参见{' '}
        ...
        <form className="auth-form" onSubmit={handleSubmit}>
          ...
          {state.mode !== 'reset' ? (
            <label className="field">
              ...
              <input
                type="password"
                name="password"
                autoComplete={state.mode === 'signin' ? 'current-password' : 'new-password'}
                placeholder="至少 8 位"
                value={state.password}
                onChange={(event) => setState((previous) => ({ ...previous, password: event.target.value }))}
              />
            </label>
          ) : null}
          ...
          <button type="submit" className="button button-primary" disabled={state.loading}>
            {state.mode === 'signin' ? <LogIn size={16} aria-hidden="true" /> : null}
            {state.mode === 'signup' ? <UserPlus size={16} aria-hidden="true" /> : null}
            {state.mode === 'reset' ? <Mail size={16} aria-hidden="true" /> : null}
