...
export function calculateAccuracy(logs: ReviewLog[]): number {
  ...
  return Math.round((logs.filter((log) => log.answeredCorrectly).length / logs.length) * 100)
}
...
export function calculateTodayStudyMinutes(logs: ReviewLog[], sessions: StudySession[], now = Date.now()): number {
  const dayStart = startOfToday(now)
  const todayLogs = logs.filter((log) => log.reviewedAt >= dayStart)
  const todaySessions = sessions.filter(
    (session) => session.completedAt !== null && session.completedAt >= dayStart
  )

  const logMinutes = (todayLogs.reduce((total, log) => total + log.elapsedMs, 0) / 60000)
...
export function recentActivity(logs: ReviewLog[], days = 7, now = Date.now()): Array<{ date: string; count: number }> {
  const dayStart = startOfToday(now)
